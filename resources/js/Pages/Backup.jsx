...
export default function Backup() {
    ...
    useEffect(() => {
        fetch('http://localhost:8000/api/admin/backup/report')
            ...
            .then(data => {
                const filtered = data.data.filter(item => item.type === 'Backup');
                setBackups(filtered);
            });
    }, []);
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Agent Backup</h2>}
        >
                            ...
                            <table className="min-w-full table-auto border-collapse border border-gray-300">
                                <thead>
                                    <tr className="bg-gray-200">
                                        ...
                                        <th className="border px-4 py-2">Last Backup</th>
                                ...
                                <tbody>
                                    {paginatedBackups.map((backup) => (
                                        <tr key={backup.id}>
                                            <td className="border px-4 py-2 text-center">{backup.id}</td>
